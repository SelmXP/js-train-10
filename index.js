...
function customAssign(...objects) {
  ...
  for (let name in obj) obj[name] = obj[name].toString();
  return obj;
}
...
function customObjectFromEntries(entries) {
  ...
  const entriess = entries.map(([key, value]) => {
    if (typeof key === "number") key = key.toString();
    return [key, value];
  });

  return Object.fromEntries(entriess);
}
...
function checkProperty(obj, prop) {
  ...
  if (typeof obj !== "object" || typeof prop !== "string") return false;
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  else return `Помилка: об'єкт не було "заморожено"`;
}
...
// console.log(checkOwnership({ a: 1, b: 2, c: 3 }, { a: 2, b: 2, c: 4 }, "a")); // false
// console.log(checkOwnership({ a: 1, b: 2, c: 3 }, { g: 1, b: 2, c: 4 }, "a")); // false
